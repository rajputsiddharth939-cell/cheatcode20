...
function Count() {
  ...
  useEffect(() => {
    const tick = () => setTime(Math.max(0, new Date("2026-10-11T00:00:00+05:30") - Date.now()));
    tick(); const id = setInterval(tick, 1000); return () => clearInterval(id);
  }, []);
  const values = [Math.floor(time / 864e5), Math.floor(time / 36e5) % 24, Math.floor(time / 6e4) % 60, Math.floor(time / 1e3) % 60];
  return <div className="count" aria-label="Countdown to launch">{values.map((value, index) => <div key={["days","hours","minutes","seconds"][index]}><b>{String(value).padStart(2,"0")}</b><small>{["DAYS","HOURS","MIN","SEC"][index]}</small></div>)}</div>;
}

function HeroProduct() {
  const [index, setIndex] = useState(0);
  useEffect(() => { let timer; const advance = () => { setIndex(current => (current + 1) % flavours.length); timer = setTimeout(advance, 5000); }; timer = setTimeout(advance, 5000); return () => clearTimeout(timer); }, []);
  return <div className="hero-slider" aria-label="CHEATCODE flavour showcase">{flavours.map((flavour,i) => <div className={`hero-slide ${i===index?"active":""}`} key={flavour.name+flavour.sub}><Product src={flavour.image}/><div className="hero-flavour-label">{flavour.name} {flavour.sub}</div></div>)}<button className="hero-prev" onClick={() => setIndex(current => (current-1+flavours.length)%flavours.length)} aria-label="Previous flavour">‹</button><button className="hero-next" onClick={() => setIndex(current => (current+1)%flavours.length)} aria-label="Next flavour">›</button><div className="hero-dots" aria-hidden="true">{flavours.map((flavour,i)=><span className={i===index?"active":""} key={flavour.name}/>)}</div></div>;
}
...
<section className="black flavours-sec" id="flavours"><div className="wrap"><div className="center"><span className="eyebrow redtext">02 / THE LINE-UP</span><h2>5 WAYS TO CHEAT.</h2><p className="muted">Different moods. Same CHEATCODE.</p></div><div className="flavour-row">{flavours.map((flavour,index)=><article key={flavour.name+"-"+flavour.sub}><img className="mini-pack" src={flavour.image} alt={flavour.name+" "+flavour.sub+" CHEATCODE ice cream"} loading="lazy" decoding="async"/><span>0{index+1}</span><h3>{flavour.name}<br/>{flavour.sub}</h3></article>)}</div><div className="benefits">{[["10g","Protein"],["0","Added Sugar"],["High","Fibre"],["Low","Carb"]].map(([title,label])=><div key={label}><i>◉</i><b>{title}</b><small>{label}</small></div>)}</div></div></section>
...
<Signup/><section className="black follow"><div className="wrap"><span className="eyebrow redtext">08 / FOLLOW THE BUILD</span><h2>@HOUSEOFCHEATCODE</h2><p className="muted">Flavour testing. Packaging. First batches. Launch preparation. The real build, before the first scoop.</p><a className="outline" href="https://www.instagram.com/houseofcheatcode/" target="_blank" rel="noreferrer">FOLLOW ON INSTAGRAM ↗</a></div></section><section className="red final"><div className="wrap"><span className="eyebrow">09 / CHEATCODE™</span><h2>YOUR NEXT<br/>CHEAT IS<br/>ALMOST HERE.</h2><p>11.10.26</p></div></section></main><footer><b>CHEATCODE™</b><span>You Can Cheat Without Regret.</span><a href="mailto:hello@houseofcheatcode.com">HELLO@HOUSEOFCHEATCODE.COM</a></footer></div>}
